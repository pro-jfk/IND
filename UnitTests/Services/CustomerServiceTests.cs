...
public class CustomerServiceTests
{
    ...
    public async Task VerifyFingerprint_FingerprintMatched()
    {
        ...
        var customer = new Customer { Id = id,Salt = Encoding.UTF8.GetBytes("salt"), FingerPrint = "hashedFingerprint" };
        _customerRepositoryMock.Setup(x => x.GetFirstASync(c => c.Id == id)).ReturnsAsync(customer);
        _hashServiceMock.Setup(x => x.Verify(fingerprint, customer.Salt, customer.FingerPrint)).ReturnsAsync(true);
        ...
        _hashServiceMock.Verify(x => x.Verify(fingerprint, customer.Salt, customer.FingerPrint), Times.Once);
        Assert.True(result);
    }
    ...
    [Fact]
    public async Task VerifyFingerprint_Should_Fail_Verification()
    {
        // Arrange
        var id = 1;
        var fingerprint = "fingerprint";
        var customer = new Customer { Id = id, Salt = Encoding.UTF8.GetBytes("salt"), FingerPrint = "hashedFingerprint" };
        _customerRepositoryMock.Setup(x => x.GetFirstASync(c => c.Id == id)).ReturnsAsync(customer);
        _hashServiceMock.Setup(x => x.Verify(fingerprint, customer.Salt, customer.FingerPrint)).ReturnsAsync(false);
